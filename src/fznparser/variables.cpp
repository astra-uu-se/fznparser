...
const std::string_view& BoolVar::identifier() const { return _identifier; }
...
bool BoolVar::operator==(const BoolVar& other) const {
  std::cout << "BoolVar: comparing: " << toString() << " and "
            << other.toString() << '\n';
  if (_identifier != other.identifier() || _domain != other._domain ||
      _annotations.size() != other._annotations.size()) {
    return false;
  }
  for (size_t i = 0; i < _annotations.size(); ++i) {
    if (_annotations.at(i).operator!=(other._annotations.at(i))) {
      return false;
    }
  }
  return true;
...
std::string BoolVar::toString() const {
  std::string s = "var bool: " + std::string(_identifier);
  if (_domain == 1) {
    s += " = true";
  } else if (_domain == 0) {
    s += " = false";
...
IntVar::IntVar(std::vector<int64_t>&& vals, const std::string_view& identifier,
    ...
    : _identifier(identifier),
      _annotations(std::move(annotations)),
      _domain(std::move(vals)) {}

const std::string_view& IntVar::identifier() const { return _identifier; }
...
void IntVar::addAnnotation(const std::string_view& identifier) {
  _annotations.push_back(Annotation(identifier));
}

void IntVar::addAnnotation(const Annotation& annotation) {
...
bool IntVar::operator==(const IntVar& other) const {
  std::cout << "IntVar: comparing: " << toString() << " and "
            << other.toString() << '\n';
  if (_identifier != other.identifier() || _domain.operator!=(other._domain) ||
      _annotations.size() != other._annotations.size()) {
    return false;
  }
  for (size_t i = 0; i < _annotations.size(); ++i) {
...
std::string IntVar::toString() const {
  std::string s = "var " + _domain.toString() + ": " + std::string(_identifier);
  if (!_annotations.empty()) {
    for (const auto& annotation : _annotations) {
      s += " :: " + annotation.toString();
    }
...
const std::string_view& FloatVar::identifier() const { return _identifier; }
...
bool FloatVar::operator==(const FloatVar& other) const {
  std::cout << "FloatVar: comparing: " << toString() << " and "
            << other.toString() << '\n';
  if (_identifier != other.identifier() || _domain.operator!=(other._domain) ||
      _annotations.size() != other._annotations.size()) {
    return false;
...
const std::string_view& SetVar::identifier() const { return _identifier; }
...
bool SetVar::operator==(const SetVar& other) const {
  std::cout << "SetVar: comparing: " << toString() << " and "
            << other.toString() << '\n';
  if (_identifier != other.identifier() || _domain.operator!=(other._domain) ||
      _annotations.size() != other._annotations.size()) {
    return false;
...
const std::string_view& BoolVarArray::identifier() const { return _identifier; }

bool BoolVarArray::isParArray() const {
  return std::all_of(this->begin(), this->end(), [&](const auto& var) {
    return std::holds_alternative<bool>(var);
  });
}

std::vector<bool> BoolVarArray::toParVector() const {
  ...
  for (size_t i = 0; i < this->size(); ++i) {
    ...
    params.emplace_back(get<bool>((*this)[i]));
  }
  return params;
}
...
void BoolVarArray::addArgument(bool b) { emplace_back(b); }
void BoolVarArray::addArgument(const BoolVar& var) {
  emplace_back(reference_wrapper<const BoolVar>(var));
}

bool BoolVarArray::operator==(const BoolVarArray& other) const {
  std::cout << "BoolVarArray: comparing: " << toString() << " and "
            << other.toString() << '\n';
  if (_identifier != other._identifier || size() != other.size() ||
      _annotations.size() != other._annotations.size()) {
    return false;
  }
...
std::string BoolVarArray::toString() const {
  std::string s = _identifier.empty()
                      ? "["
                      : ("array[1.." + std::to_string(size()) + "] of" +
                         (isParArray() ? "" : " var") +
                         " bool: " + std::string(_identifier) + " = [");
  for (size_t i = 0; i < size(); ++i) {
    if (i != 0) {
      s += ", ";
    }
...
const std::string_view& IntVarArray::identifier() const { return _identifier; }

bool IntVarArray::isParArray() const {
  return std::all_of(this->begin(), this->end(), [&](const auto& var) {
    return std::holds_alternative<int64_t>(var);
  });
}

std::vector<int64_t> IntVarArray::toParVector() const {
  ...
  for (size_t i = 0; i < this->size(); ++i) {
    ...
    params.emplace_back(get<int64_t>((*this)[i]));
  }
  return params;
}
...
void IntVarArray::addArgument(int64_t i) { emplace_back(i); }
void IntVarArray::addArgument(const IntVar& var) {
  emplace_back(reference_wrapper<const IntVar>(var));
}

void IntVarArray::addAnnotation(const std::string_view& identifier) {
  _annotations.push_back(Annotation(identifier));
}

void IntVarArray::addAnnotation(const Annotation& annotation) {
  _annotations.push_back(annotation);
}

void IntVarArray::addAnnotation(
    const std::string_view& identifier,
    std::vector<AnnotationExpression>&& expression) {
  _annotations.push_back(Annotation(
      identifier, std::move(std::vector<std::vector<AnnotationExpression>>{
...
bool IntVarArray::operator==(const IntVarArray& other) const {
  std::cout << "IntVarArray: comparing: " << toString() << " and "
            << other.toString() << '\n';
  if (_identifier != other._identifier || size() != other.size() ||
      _annotations.size() != other._annotations.size()) {
    return false;
...
const std::string_view& FloatVarArray::identifier() const {
  return _identifier;
}

bool FloatVarArray::isParArray() const {
  return std::all_of(this->begin(), this->end(), [&](const auto& var) {
    return std::holds_alternative<double>(var);
  });
}

void FloatVarArray::addArgument(double d) { emplace_back(d); }
void FloatVarArray::addArgument(const FloatVar& var) {
  emplace_back(reference_wrapper<const FloatVar>(var));
}
...
std::vector<double> FloatVarArray::toParVector() const {
  ...
  for (size_t i = 0; i < this->size(); ++i) {
    ...
    params.emplace_back(get<double>((*this)[i]));
  }
  return params;
}

bool FloatVarArray::operator==(const FloatVarArray& other) const {
  std::cout << "FloatVarArray: comparing: " << toString() << " and "
            << other.toString() << '\n';
  if (_identifier != other._identifier || size() != other.size() ||
      _annotations.size() != other._annotations.size()) {
    return false;
...
const std::string_view& SetVarArray::identifier() const { return _identifier; }

bool SetVarArray::isParArray() const {
  return std::all_of(this->begin(), this->end(), [&](const auto& var) {
    return std::holds_alternative<IntSet>(var);
  });
}
...
std::vector<IntSet> SetVarArray::toParVector() const {
  std::vector<IntSet> params;
  params.clear();
  for (size_t i = 0; i < this->size(); ++i) {
    if (!std::holds_alternative<IntSet>((*this)[i])) {
      throw std::runtime_error("Cannot convert to parameter array");
    }
    params.emplace_back(get<IntSet>((*this)[i]));
  }
  return params;
}

void SetVarArray::addArgument(IntSet&& is) { emplace_back(std::move(is)); }
void SetVarArray::addArgument(const SetVar& var) {
  emplace_back(reference_wrapper<const SetVar>(var));
}

bool SetVarArray::operator==(const SetVarArray& other) const {
  std::cout << "SetVarArray: comparing: " << toString() << " and "
            << other.toString() << '\n';
  if (_identifier != other._identifier || size() != other.size() ||
      _annotations.size() != other._annotations.size()) {
    return false;
...
std::string SetVarArray::toString() const {
  ...
  for (size_t i = 0; i < size(); ++i) {
    ...
    if (holds_alternative<IntSet>(at(i))) {
      s += get<IntSet>(at(i)).toString();
    } else {
      s += get<reference_wrapper<const SetVar>>(at(i)).get().identifier();
    }
  }
  s += "]";
  if (!_annotations.empty()) {
...
bool Variable::operator==(const Variable& other) const {
  std::cout << "Variable: comparing: " << toString() << " and "
            << other.toString() << '\n';
  if (std::holds_alternative<BoolVar>(*this) &&
